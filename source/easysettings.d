 ...
 */
auto loadSettings(T, alias settingsFormat = SettingsFormat)(string name, string filename = settingsFilename) {
	...
	auto searchPaths = standardPaths(StandardPath.config, name).chain(["."]).cartesianProduct(only(SettingsExtensions!settingsFormat)).map!(x => chainPath(x[0], filename.withExtension(x[1])));
	auto paths = searchPaths.filter!exists;
 ...
 */
void saveSettings(T, alias settingsFormat = SettingsFormat)(T data, string name, string filename = settingsFilename) {
	import std.path : buildPath, setExtension;
	import std.file : exists, mkdirRecurse;
	string configPath = writablePath(StandardPath.config, name);
 ...
 */
void deleteSettings(alias settingsFormat = SettingsFormat)(string name, string filename = settingsFilename) {
	import std.path : buildPath, dirName, setExtension;
	import std.file : exists, remove, dirEntries, SpanMode, rmdir;
	auto path = buildPath(writablePath(StandardPath.config), name, filename.setExtension(SettingsExtensions!settingsFormat[0]));
	if (path.exists) {
		remove(path);
	}
